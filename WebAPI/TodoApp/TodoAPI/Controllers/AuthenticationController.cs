...
public class AuthenticationController : ControllerBase
{
    ...
    {
        // not production code, will replace with real auth system
        if (CompareValues(data.UserName, "devjay") &&
            CompareValues(data.Password, "jay123"))
        {
            return new UserData(1, "Jason", "James", data.UserName!);
        }

        if (CompareValues(data.UserName, "homer") &&
            CompareValues(data.Password, "homer123"))
        {
            return new UserData(1, "Homer", "Simpson", data.UserName!);
        }
